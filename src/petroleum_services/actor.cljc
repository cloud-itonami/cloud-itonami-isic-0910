(ns petroleum-services.actor
  "PetroleumServicesActor — the ISIC-08 0910 petroleum services contractor
  ...
  The unconditional invariant: the PetroleumServicesAdvisor can never
  directly commit a record or dispatch any operation the
  PetroleumServicesGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [petroleum-services.advisor :as advisor]
            [petroleum-services.governor :as governor]
            [petroleum-services.store :as store]))

(defn build-graph
  "Build a compiled PetroleumServicesActor graph. `store` implements
  `petroleum-services.store/Store`. `advisor` implements
  `petroleum-services.advisor/Advisor` (defaults to `mock-advisor`).
